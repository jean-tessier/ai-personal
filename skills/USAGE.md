...
whether the target is a skill (`skills/<name>/`) or an agent prompt (`prompts/agents/<name>/`),
confirms it's a real asset (`SKILL.md` or `system.md` present), asks one focused question if
you didn't describe the change, and appends a newest-first bullet to `CHANGELOG.md` (creating
it with the standard header if it doesn't exist yet).
